...
class BarcodeAnalyzer(private val cameraExecutor: ExecutorService, private val context: Context) : ImageAnalysis.Analyzer {
    ...
    private fun scanBarcodes(imageProxy: ImageProxy) {
        ...
        if (mediaImage != null) {
            ...
            val options = BarcodeScannerOptions.Builder().setBarcodeFormats(Barcode.FORMAT_EAN_13).build()
    ...
    private fun passISBN(stringISBN: String) {
        val intent = Intent(context, FillSaleActivity::class.java).apply { putExtra(ISBN, stringISBN) }
